...
const DraggableRow: React.FC<RowProps> = ({ y, type, name }) => {
  ...
  const bind = useDrag(
    ...
    { 
      from: () => [x.get() * 40, 0],
      axis: 'x', // Lock gesture to X axis to prevent vertical scrolling interference
      filterTaps: true, // Ignore taps, only drags
    }
  );
  ...
  return (
    <group name={name} position={[0, y, 0]} {...bind() as any}>
      {/* 
         Invisible plane to catch drag events.
         IMPORTANT: Must NOT be visible={false} on the mesh itself, or raycaster ignores it.
         Use a transparent material instead.
         
         UPDATED: Reduced height to 1.0 to prevent overlapping with adjacent rows.
         UPDATED: Moved to z=2 to ensure it is in front of the models.
      */}
      <mesh position={[0, 0, 2]}>
